"""
Main Scraper for OpenSooq Businesses & Industrial Category
...
from .config import (
    BASE_URL, BUSINESSES_URL, HEADERS,
...
class BusinessesIndustrialScraper:
    """Main scraper class for Businesses & Industrial listings"""
    
    def __init__(self, s3_uploader: BusinessesS3Uploader):
        """
        Initialize scraper
    ...
    def get_main_categories(self) -> List[Dict]:
        """
        Get main categories from Businesses & Industrial page
        
        Returns:
            List of main categories with details
        """
        html = self.fetch_page(BUSINESSES_URL)
        if not html:
            logger.error("Failed to fetch main Businesses & Industrial page")
            return []
        
        try:
            page_props = extract_json_from_html(html, 'pageProps')
            if not page_props:
                logger.error("Failed to extract JSON from Businesses page")
                return []
    ...
    def scrape_all_businesses(self):
        """
        Main method to scrape all Businesses & Industrial categories
        """
        logger.info(f"Starting Businesses & Industrial scrape for date: {self.target_date}")
...
def main():
    ...
    s3_uploader = BusinessesS3Uploader(
    ...
    scraper = BusinessesIndustrialScraper(s3_uploader)
